...
**Scope boundary:** This skill owns general .NET Native AOT compilation -- the publish pipeline, MSBuild configuration, type preservation, P/Invoke, and size optimization. MAUI-specific iOS/Mac Catalyst AOT (publish profiles, platform-specific library compat) is owned by fn-14 -- see [skill:dotnet-maui-aot]. AOT-first application design patterns (source gen over reflection, DI, serialization choices) are in [skill:dotnet-aot-architecture]. Trim-safe library authoring is in [skill:dotnet-trimming]. WASM AOT for Blazor/Uno is in [skill:dotnet-aot-wasm].

**Out of scope:** MAUI iOS/Mac Catalyst AOT pipeline -- see [skill:dotnet-maui-aot] (fn-14). Source generator authoring (Roslyn API) -- see [skill:dotnet-csharp-source-generators] (fn-3). DI container patterns -- see [skill:dotnet-csharp-dependency-injection] (fn-3). Serialization depth -- see [skill:dotnet-serialization] (fn-6). Container deployment orchestration -- see [skill:dotnet-containers] (fn-5).
