...
**Scope boundary:** This skill owns artifact publishing pipelines from Azure DevOps. Container image authoring (Dockerfile best practices, SDK container properties) is owned by [skill:dotnet-containers] (fn-5). Native AOT compilation configuration is owned by [skill:dotnet-native-aot] (fn-16) -- this skill references AOT for CI pipeline configuration only. CLI-specific release pipelines are owned by [skill:dotnet-cli-release-pipeline] (fn-17). Starter CI templates are owned by [skill:dotnet-add-ci] (fn-4).

**Out of scope:** Container image authoring (Dockerfile, base image selection) -- see [skill:dotnet-containers] (fn-5). Native AOT MSBuild configuration -- see [skill:dotnet-native-aot] (fn-16). CLI release pipelines -- see [skill:dotnet-cli-release-pipeline] (fn-17). Starter CI templates -- see [skill:dotnet-add-ci] (fn-4). GitHub Actions publishing -- see [skill:dotnet-gha-publish] (fn-19). ADO-unique features (environments, service connections) -- see [skill:dotnet-ado-unique] (fn-19).
