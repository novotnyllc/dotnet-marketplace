...
**Scope boundary:** This skill owns deployment pipeline patterns for GitHub Actions. Container orchestration and runtime configuration are owned by [skill:dotnet-container-deployment] (fn-5). Container image authoring is owned by [skill:dotnet-containers] (fn-5). Publishing (NuGet push, container build) is in `dotnet-gha-publish` (fn-19). Composable workflow patterns are in `dotnet-gha-patterns` (fn-19). Starter CI templates are owned by [skill:dotnet-add-ci] (fn-4).

**Out of scope:** Container orchestration (Kubernetes, Docker Compose) -- see [skill:dotnet-container-deployment] (fn-5). Container image authoring -- see [skill:dotnet-containers] (fn-5). NuGet publishing and container builds -- see `dotnet-gha-publish` (fn-19). Starter CI templates -- see [skill:dotnet-add-ci] (fn-4). Azure DevOps deployment -- see `dotnet-ado-patterns` (fn-19). CLI release pipelines -- see [skill:dotnet-cli-release-pipeline] (fn-17).
