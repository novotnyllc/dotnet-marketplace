...
**Scope boundary:** This skill owns ADO-exclusive platform features that have no direct GitHub Actions equivalent. Composable YAML pipeline patterns (templates, triggers, multi-stage) are in `dotnet-ado-patterns` (fn-19). Build/test pipeline configuration is in `dotnet-ado-build-test` (fn-19). Publishing pipelines are in `dotnet-ado-publish` (fn-19). Starter CI templates are owned by [skill:dotnet-add-ci] (fn-4).

**Out of scope:** Composable pipeline patterns (templates, triggers) -- see `dotnet-ado-patterns` (fn-19). Build/test pipeline configuration -- see `dotnet-ado-build-test` (fn-19). Publishing pipelines -- see `dotnet-ado-publish` (fn-19). Starter CI templates -- see [skill:dotnet-add-ci] (fn-4). GitHub Actions equivalents -- see `dotnet-gha-patterns`, `dotnet-gha-build-test`, `dotnet-gha-publish`, `dotnet-gha-deploy` (fn-19). CLI release pipelines -- see [skill:dotnet-cli-release-pipeline] (fn-17).
