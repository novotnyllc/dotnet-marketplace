...
**Out of scope:** C# language syntax for Span, records, pattern matching, and collection expressions is owned by fn-3 -- see [skill:dotnet-csharp-modern-patterns]. Coding standards and naming conventions (including sealed class style guidance) are owned by fn-3 -- see [skill:dotnet-csharp-coding-standards]. Microbenchmarking setup and measurement is owned by this epic's companion skill -- see [skill:dotnet-benchmarkdotnet]. Native AOT compilation pipeline and trimming are owned by fn-16 -- see [skill:dotnet-native-aot]. Serialization format performance tradeoffs are owned by fn-6 -- see [skill:dotnet-serialization]. Architecture patterns (caching, resilience, DI) are owned by fn-5 -- see [skill:dotnet-architecture-patterns]. EF Core query optimization is owned by fn-5 -- see [skill:dotnet-efcore-patterns].
