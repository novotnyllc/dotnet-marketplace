...
description: "WHEN microbenchmarking .NET code. BenchmarkDotNet setup, memory diagnosers, exporters, baselines, CI integration."
---
...
**Out of scope:** Performance-oriented architecture patterns (Span\<T\>, ArrayPool\<T\>, sealed class devirtualization) are owned by this epic's companion skill -- see [skill:dotnet-performance-patterns]. C# syntax for modern patterns (records, primary constructors) is owned by fn-3 -- see [skill:dotnet-csharp-modern-patterns]. Coding standards and style conventions are owned by fn-3 -- see [skill:dotnet-csharp-coding-standards]. Native AOT compilation pipeline and performance characteristics are owned by fn-16 -- see [skill:dotnet-native-aot]. Serialization format APIs and round-trip correctness are owned by fn-6 -- see [skill:dotnet-serialization]. Profiling tools (dotnet-counters, dotnet-trace, dotnet-dump) are covered by [skill:dotnet-profiling]. CI benchmark regression detection is covered by [skill:dotnet-ci-benchmarking]. Architecture patterns (caching, resilience) are owned by fn-5 -- see [skill:dotnet-architecture-patterns]. EF Core query optimization is owned by fn-5 -- see [skill:dotnet-efcore-patterns].
