---
name: dotnet-msix
description: "WHEN packaging MSIX apps. Creation, certificate signing, Store submission, App Installer sideload, auto-update, bundles."
---
...
**Scope boundary:** This skill owns the MSIX packaging pipeline -- creation, signing, distribution, Store submission, auto-update, and CI build steps. WinUI 3 project setup, XAML patterns, and the packaged vs unpackaged deployment mode comparison are owned by [skill:dotnet-winui] (fn-13). General Native AOT compilation (PublishAot, ILLink descriptors) is owned by [skill:dotnet-native-aot] (fn-16). General CI/CD pipeline patterns are owned by [skill:dotnet-gha-patterns] and [skill:dotnet-ado-patterns] (fn-19).

**Out of scope:** WinUI 3 project setup and MSIX vs unpackaged comparison -- see [skill:dotnet-winui] (fn-13). Native AOT MSBuild configuration -- see [skill:dotnet-native-aot] (fn-16). General CI/CD pipeline patterns -- see [skill:dotnet-gha-patterns] and [skill:dotnet-ado-patterns] (fn-19). General NuGet packaging -- see [skill:dotnet-nuget-authoring] (fn-20). Container-based deployment -- see [skill:dotnet-containers] (fn-5).
