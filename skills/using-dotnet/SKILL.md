...
- **Don't add what wasn't asked for.** No extra config options, no "while we're at it" refactors, no preemptive error handling for impossible scenarios, no comments or XML docs on code you didn't change.
...
- **Fewer files, fewer layers.** Don't split into Controller + Service + Repository + DTO + Mapper + Interface when one or two files will do. Add layers only when the code gets hard to understand without them.
