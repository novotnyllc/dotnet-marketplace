...
**Out of scope:** Tactical EF Core usage (DbContext lifecycle, AsNoTracking, migrations, interceptors, compiled queries) is covered in [skill:dotnet-efcore-patterns]. Choosing between EF Core, Dapper, and ADO.NET is covered in [skill:dotnet-data-access-strategy]. DI container mechanics are owned by fn-3 -- see [skill:dotnet-csharp-dependency-injection]. Async patterns are owned by fn-3 -- see [skill:dotnet-csharp-async-patterns]. Integration testing data layers is owned by fn-7 -- <!-- TODO: fn-7 reconciliation -- replace with [skill:dotnet-integration-testing] when fn-7 lands -->. CI/CD pipelines are owned by fn-19.
