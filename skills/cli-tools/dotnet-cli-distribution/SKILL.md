...
description: "WHEN shipping CLI tools. AOT vs framework-dependent vs dotnet tool, RID matrix, single-file publish, size tuning."
---
...
**Out of scope:** Native AOT MSBuild configuration (PublishAot, ILLink descriptors, EnableAotAnalyzer, trimming) -- see [skill:dotnet-native-aot] (fn-16). AOT-first application design patterns (source gen over reflection, DI choices) -- see [skill:dotnet-aot-architecture] (fn-16). Multi-platform packaging formats (Homebrew, apt/deb, winget, Scoop) -- see [skill:dotnet-cli-packaging] (fn-17). Release CI/CD pipeline -- see [skill:dotnet-cli-release-pipeline] (fn-17). Container-based distribution -- see [skill:dotnet-containers] (fn-5). General CI/CD patterns -- see [skill:dotnet-gha-patterns] and [skill:dotnet-ado-patterns].
