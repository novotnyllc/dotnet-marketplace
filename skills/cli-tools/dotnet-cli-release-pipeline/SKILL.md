...
description: "WHEN releasing CLI tools. GitHub Actions build matrix, artifact staging, GitHub Releases, checksums, package PRs."
---
...
**Scope boundary:** This skill owns the CLI-specific release pipeline -- the build-package-release workflow for CLI tool artifacts. General CI/CD patterns (branch protection, matrix testing strategies, deployment pipelines, reusable workflows) are owned by fn-19. This skill focuses on the unique requirements of shipping CLI binaries to multiple package managers from a single trigger.

**Out of scope:** General CI/CD patterns (branch strategies, matrix testing, deployment pipelines) -- see fn-19 CI/CD skills. Native AOT compilation configuration -- see [skill:dotnet-native-aot] (fn-16). Distribution strategy decisions -- see [skill:dotnet-cli-distribution] (fn-17). Package format details -- see [skill:dotnet-cli-packaging] (fn-17). Container image publishing -- see [skill:dotnet-containers] (fn-5).
