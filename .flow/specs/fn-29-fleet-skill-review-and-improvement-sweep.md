...
Deploy a fleet of parallel worker agents to review all 101 skills in dotnet-artisan against best practices from Anthropic's skill authoring guide and this repo's conventions. Each agent reviews a batch of skills by category, produces a findings report, and then implements approved improvements. A coordinator task merges findings and resolves cross-cutting issues.
...
5. **Cross-References** — Does it use `[skill:name]` syntax for all skill references? Are cross-references accurate and pointing to existing skills?
...
9. **Consistency** — Does it follow the same patterns as peer skills in its category?

### Category Batches for Parallel Workers

Batches are designed so each worker handles ~15-20 skills:
...
- The evaluation rubric (9 dimensions above) with scoring guidance
- Per-skill output template (structured markdown for findings)
- Category batch assignments
...
Apply approved changes to skills in Batches A and B. Commit per-category.

### Task 10: Implement Improvements — Batches C+D

Apply approved changes to skills in Batches C and D. Commit per-category.

### Task 11: Implement Improvements — Batches E+F

Apply approved changes to skills in Batches E and F. Commit per-category.
...
- Update plugin.json if any skills were added/removed/renamed
...
- [ ] All 101 skills reviewed against the 9-dimension rubric
- [ ] Findings reports produced for all 6 batches
- [ ] Consolidated improvement plan prioritized
